...
def merge_post_vcfs(
    ref, resolved_vcf, no_resolve_vcf, out_vcf, pass_threshold, lowqual_threshold
):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ref", help="reference fasta filename", required=True)
    parser.add_argument("--resolved_vcf", help="resolved_vcf", required=True)
    parser.add_argument("--no_resolve_vcf", help="no resolve vcf", required=True)
    parser.add_argument("--out_vcf", help="output vcf", required=True)
    args = parser.parse_args()
    logger.info(args)
    try:
        merge_post_vcfs(args.ref, args.resolved_vcf, args.no_resolve_vcf, args.out_vcf)
    except Exception as e:
        logger.error(traceback.format_exc())
        logger.error("Aborting!")
        logger.error("merge_post_vcfs.py failure on arguments: {}".format(args))
        raise e
