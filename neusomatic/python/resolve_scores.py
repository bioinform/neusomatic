...
def resolve_scores(input_bam, ra_vcf, target_vcf, output_vcf):
    ...
    tmp_ = bedtools_window(ra_vcf, target_vcf, args=" -w 5 -v", run_logger=logger)

    final_intervals = read_tsv_file(tmp_)
    for x in final_intervals:
        x[5] = str(np.round(-10 * np.log10(0.25), 4))

    tmp_ = bedtools_window(ra_vcf, target_vcf, args=" -w 5", run_logger=logger)
    ...
    with open(tmp_) as i_f:
        for line in skip_empty(i_f):
            ...
            id_ = "{}-{}-{}-{}".format(
                interval[0], interval[1], interval[3], interval[4]
            )
    ...
    for id_, intervals in intervals_dict.items():
        if len(intervals) == 1:
            ...
            len_diff = list(
                map(lambda x: abs((len(x[14]) - len(x[13])) - len_), intervals)
            )
            min_len_diff = min(len_diff)
            intervals = list(
                filter(
                    lambda x: abs((len(x[14]) - len(x[13])) - len_) == min_len_diff,
                    intervals,
                )
            )
            ...
            intervals = list(
                filter(lambda x: abs(int(x[11]) - pos_) == min_pos_diff, intervals)
            )
            score = "{:.4f}".format(
    ...
    out_variants = sorted(
        final_intervals, key=lambda x: [chroms_order[x[0]], int(x[1])]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ra_vcf", type=str, help="realigned vcf", required=True)
    parser.add_argument("--target_vcf", type=str, help="target vcf", required=True)
    parser.add_argument("--output_vcf", type=str, help="output_vcf", required=True)
    args = parser.parse_args()

    try:
        resolve_scores(args.input_bam, args.ra_vcf, args.target_vcf, args.output_vcf)
    except Exception as e:
        logger.error(traceback.format_exc())
        logger.error("Aborting!")
        logger.error("resolve_scores.py failure on arguments: {}".format(args))
        raise e
