...
def merge_tsvs(
    ...
    candidates_per_tsv = max(
        candidates_per_tsv, np.ceil(totla_L / float(max_num_tsvs)) + 1
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_tsvs", nargs="*", help=" input candidate tsv files", required=True
    )
    ...
    parser.add_argument(
        ...
        help="Maximum number of merged tsv files \
                        (higher priority than candidates_per_tsv)",
        default=10,
    )
    parser.add_argument(
        "--overwrite_merged_tsvs",
    ...
    try:
        merged_tsvs = merge_tsvs(
            args.input_tsvs,
            args.out,
            args.candidates_per_tsv,
            args.max_num_tsvs,
        ...
        logger.error("merge_tsvs.py failure on arguments: {}".format(args))
        raise e
