...
class Realign_Read:
    ...
    def fix_record(self, record, ref_seq):
        ...
        start_hc = cigartuples[0][1] if cigartuples[0][0] == CIGAR_HARDCLIP else 0
        ...
        try:
            ...
            logger.error(
                "Realignments are empty for {} at {}:{}".format(
        ...
        for (
            ...
            excess_start,
            excess_end,
        ) in self.realignments:
            c_array = np.array(
                list(
                    map(
                        lambda x: [x[0], x[1][1] if x[1][0] != CIGAR_DEL else 0],
                        enumerate(cigartuples),
                    )
                )
            )
            ...
            if excess_start > 0:
                if (
                    c_i > 0
                    and cigartuples[c_i - 1][0] == CIGAR_DEL
                    and cigartuples[c_i - 1][1] >= (excess_start)
                ):
            ...
            if excess_end > 0:
                if (
                    c_e < (len(cigartuples) - 1)
                    and cigartuples[c_e + 1][0] == CIGAR_DEL
                    and cigartuples[c_e + 1][1] >= (excess_end)
                ):
                    if excess_end > del_end:
                        del_end = excess_end
                elif not (
                    c_e == len(c_array) - 1 or cigartuples[c_e + 1][0] == CIGAR_SOFTCLIP
                ):
                    return record
            ...
            if del_start == 0:
                ...
                try:
                    ...
                    logger.error(
                        "Expect DEL (c_i) in positon {} at cigartuples {}".format(
                            c_i - 1, cigartuples
                        )
                    )
                    raise Exception
                if cigartuples[c_i - 1][1] > del_start:
                    left_cigartuple = left_cigartuple[:-1] + [
                        [cigartuples[c_i - 1][0], cigartuples[c_i - 1][1] - del_start]
                    ]
                else:
                    left_cigartuple = left_cigartuple[:-1]
            ...
            if del_end == 0:
                if end_match > (end_idx - bias):
                    right_cigartuple = [
                        [cigartuples[c_e][0], end_match - (end_idx - bias)]
                    ] + right_cigartuple
            else:
                try:
                    ...
                    logger.info(
                        "Expect DEL (c_e) in positon {} at cigartuples {}, {}".format(
                ...
                if cigartuples[c_e + 1][1] > del_end:
                    right_cigartuple = [
                        [cigartuples[c_e + 1][0], cigartuples[c_e + 1][1] - del_end]
                    ] + right_cigartuple[1:]
                else:
        ...
        if (
            len(new_cigartuples) > 2
            and new_cigartuples[-1][0] == CIGAR_SOFTCLIP
        ...
        if (
            len(new_cigartuples) > 2
            and new_cigartuples[0][0] == CIGAR_SOFTCLIP
        ...
        try:
            assert sum(
                map(lambda x: x[1] if x[0] != CIGAR_DEL else 0, new_cigartuples)
            ) == sum(
                map(lambda x: x[1] if x[0] != CIGAR_DEL else 0, record.cigartuples)
            )
        except:
            logger.error(
                "Old and new cigarstrings have different lengthes: {} vs {}".format(
                    sum(
                        map(lambda x: x[1] if x[0] != CIGAR_DEL else 0, new_cigartuples)
                    ),
                    sum(
                        map(
                            lambda x: x[1] if x[0] != CIGAR_DEL else 0,
                            record.cigartuples,
                        )
                    ),
                )
        ...
        record.tags = list(filter(lambda x: x[0] != "NM", record.tags)) + [
            ("NM", int(NM))
        ]
        return record
...
def find_NM(record, ref_seq):
    ...
    sc_start = (record.cigartuples[0][0] == CIGAR_SOFTCLIP) * record.cigartuples[0][1]
    sc_end = (record.cigartuples[-1][0] == CIGAR_SOFTCLIP) * record.cigartuples[-1][1]
    q_seq = record.seq
    ...
    q_array = np.array(list(map(lambda x: NUC_to_NUM[x.upper()], list(q_seq))))[non_ins]
    cigar_stat = get_cigar_stat(record.cigartuples, [CIGAR_DEL, CIGAR_INS])
    assert ref_array.shape[0] == q_array.shape[0]
    NM = (
        sum(abs(ref_array - q_array) > 0)
        + cigar_stat[CIGAR_DEL]
        + cigar_stat[CIGAR_INS]
    )
    return NM
...
def cigarstring_to_tuple(cigarstring):
    ...
    return tuple(
        (_CIGAR_OP_DICT[op], int(length))
        for length, op in _CIGAR_PATTERN.findall(cigarstring)
    )
...
def prepare_fasta(
    work, region, input_bam, ref_fasta_file, include_ref, split_i, ds, filter_duplicate
):
    ...
    info_file = os.path.join(
        work, region.__str__() + "_split_{}".format(split_i) + ".txt"
    )
                ...
                with pysam.Samfile(input_bam, "rb") as samfile:
                    for record in samfile.fetch(
                        ...
                        if np.random.rand() > ds:
                            continue
                        sc_start = (
                            record.cigartuples[0][0] == CIGAR_SOFTCLIP
                        ) * record.cigartuples[0][1]
                        sc_end = (
                            record.cigartuples[-1][0] == CIGAR_SOFTCLIP
                        ) * record.cigartuples[-1][1]
                        positions = positions[sc_start:]
                        if sc_end > 0:
                            positions = positions[:-sc_end]
                        ...
                        if max(end_idx - start_idx, pos_end - pos_start) >= (
                            region.span() * 0.75
                        ):
                            c_array = np.array(
                                list(
                                    map(
                                        lambda x: [
                                            x[0],
                                            x[1][1] if x[1][0] != CIGAR_DEL else 0,
                                        ],
                            ...
                            refseq = ref_fasta.fetch(
                                region.chrom,
                                positions_[non_ins][0],
                            ...
                            mn, mx = min(non_ins_positions), max(non_ins_positions)
                            ref_array = np.array(
                                list(map(lambda x: NUC_to_NUM[x.upper()], list(refseq)))
                            )[non_ins_positions - mn]
                            q_array = np.array(
                                list(map(lambda x: NUC_to_NUM[x.upper()], list(q_seq)))
                            )[non_ins]
...
def split_bam_to_chunks(
    ...
    with pysam.Samfile(input_bam, "rb") as samfile:
        for record in samfile.fetch(region.chrom, region.start, region.end + 1):
            ...
            sc_end = (record.cigartuples[-1][0] == CIGAR_SOFTCLIP) * record.cigartuples[
                -1
            ][1]
            positions = positions[sc_start:]
            if sc_end > 0:
...
def find_cigar(alignment):
    ...
    if sum(event_len) != alignment.shape[0]:
        logger.error(
            "event_len is different from length of alignment: {} vs {}".format(
...
def extract_new_cigars(region, info_file, out_fasta_file):
    ...
    if set(map(int, records.keys())) ^ set(range(len(records))):
        logger.error(
            "sequences are missing in the alignment {}".format(
        ...
        map(
            ...
            sorted(
                map(
                    lambda x: [
                        int(x[0]),
                        list(map(lambda x: 0 if x == "-" else 1, x[1].seq)),
                    ],
                    records.items(),
                ),
                key=lambda x: x[0],
    ...
    alignment = (
        (alignment == 0) * (1 - ref_seq) * (-1)
        + (alignment == 0) * ref_seq * CIGAR_MATCH
        + (alignment == 1) * CIGAR_INS
    ...
    for i in range(N):
        ...
        if CIGAR_MATCH in alignment[i, :]:
            ...
            excess_end[i + 1] = (region.end - info[i + 1].pos_end) - (
                max(pos_ref)
                - (pos_ref[np.where((alignment[i, :] == CIGAR_MATCH))[0][-1]])
            )
        else:
            excess_start[i + 1] = info[i + 1].pos_start - region.start
            excess_end[i + 1] = region.end - info[i + 1].pos_end
...
def extract_consensus(region, out_fasta_file):
    ...
    if len(records) <= 1:
        return {}, {}
    try:
        assert not set(map(int, records.keys())) ^ set(range(1, len(records) + 1))
    except:
        logger.error(
            "sequences are missing in the alignment {}".format(
...
def get_final_msa(region, msa_0, consensus, out_fasta_file_1, out_fasta_file_final):
    ...
    new_consensus_array = np.insert(consensus_array, inser_new_cols, 100, axis=0)
    ...
    with open(out_fasta_file_final, "w") as out_fasta:
        for i, seq in enumerate(msa.tolist()):
            out_fasta.write(">%s\n" % i)
            out_fasta.write("%s\n" % "".join(map(lambda y: NUM_to_NUC[y], seq)))
    return True
...
def merge_cigartuples(tuple1, tuple2):
    ...
    if tuple1[-1][0] == tuple2[0][0]:
        return (
            tuple1[:-1] + [[tuple1[-1][0], tuple1[-1][1] + tuple2[0][1]]] + tuple2[1:]
        )
...
def find_realign_dict(realign_bed_file, chrom):
    ...
    with open(realign_bed) as r_f:
        for line in skip_empty(r_f):
            ...
            if q_key not in realign_dict:
                realign_dict[q_key] = Realign_Read(query_name, chrom, pos, cigarstring)
            realign_dict[q_key].add_realignment(
                start,
                end,
                start_idx,
                end_idx,
    ...
    chrom_regions = sorted(map(lambda x: list(map(int, x.split("-"))), chrom_regions))
...
def correct_bam_chrom(input_record):
    ...
    thread_logger = logging.getLogger(
        "{} ({})".format(
            correct_bam_chrom.__name__, multiprocessing.current_process().name
        )
    )
    try:
        ...
        with pysam.AlignmentFile(input_bam, "rb") as samfile:
            with pysam.AlignmentFile(out_sam, "w", template=samfile) as out_samfile:
                realign_dict, chrom_regions = find_realign_dict(realign_bed_file, chrom)
                if chrom_regions:
                    region_cnt = 0
                ...
                for record in samfile.fetch(chrom):
                    ...
                    if (
                        not done_regions
                        and in_active_region
                        and record.pos > next_region[1]
                    ):
...
def parallel_correct_bam(
    work, input_bam, output_bam, ref_fasta_file, realign_bed_file, num_threads
):
    ...
    if num_threads > 1:
        ...
        correct_bam_all(work, input_bam, output_bam, ref_fasta_file, realign_bed_file)
...
def do_realign(region, info_file, max_realign_dp, thr_realign=0.0135):
    ...
    if (c < max_realign_dp) and (
        (sum_nm_snp + sum_nm_indel) / float(c + eps) / float(region.span() + eps)
        > thr_realign
    ):
        return True
    return False
...
def find_var(out_fasta_file, snp_min_af, del_min_af, ins_min_af, scale_maf, simplify):
            ...
            map(
                ...
                sorted(
                    map(
                        lambda x: [
                            int(x[0]),
                            list(map(lambda x: NUC_to_NUM[x.upper()], x[1].seq)),
                        ],
                        records.items(),
                    ),
                    key=lambda x: x[0],
    ...
    for i in range(alignment.shape[1]):
        ...
        if (
            (alt_base != "-" and ref_base == "-" and af > ins_min_af)
            or (alt_base != "-" and ref_base != "-" and af > snp_min_af)
            or (alt_base == "-" and ref_base != "-" and af > del_min_af)
        ):
    ...
    afs = np.array(afs)
    ref_seq_ = "".join(map(lambda x: NUM_to_NUC[x], filter(lambda x: x > 0, ref_seq)))
    alt_seq_ = "".join(map(lambda x: NUM_to_NUC[x], filter(lambda x: x > 0, alt_seq)))
    if not simplify:
        ...
        for i, (r, a) in enumerate(zip(list(ref_seq) + [0], list(alt_seq) + [0])):
            ...
            if done:
                if current_alt:
                    rr = "".join(
                        map(
                            lambda x: NUM_to_NUC[x],
                            filter(lambda x: x > 0, current_ref),
                        )
                    )
                    aa = "".join(
                        map(
                            lambda x: NUM_to_NUC[x],
                            filter(lambda x: x > 0, current_alt),
                        )
                    )
...
def run_realignment(input_record):
    ...
    thread_logger = logging.getLogger(
        "{} ({})".format(
            run_realignment.__name__, multiprocessing.current_process().name
        )
    )
                    ...
                    if ref_seq != alt_seq:
                        ...
                        if pos > 1:
                            ...
                            before = ref_fasta.fetch(
                                region.chrom, pos - num_add_before, pos - 1
                            ).upper()
                            pos -= num_add_before - 1
                            ref = before + ref
                            alt = before + alt
                        ref, alt, pos = TrimREFALT(ref, alt, pos)
                        a = int(np.ceil(np.max(afs) * len(afss)))
                        af = sum(
                            sorted(
                                map(lambda x: np.max(x) if x.shape[0] > 0 else 0, afss)
                            )[-a:]
                        ) / float(len(afss))
                        ...
                        if alt == "" and pos > 1:
                            ...
                            ref = r_ + ref
                            alt = r_ + alt
                        variants.append([region.chrom, pos, ref, alt, dp, ro, ao])
            else:
                if skipped > 0:
    ...
    with pysam.Fastafile(ref_fasta_file) as ref_fasta:
        ...
        with open(region_bed_file) as r_f:
            for line in skip_empty(r_f):
                ...
                s_base = ref_fasta.fetch(chrom, start - pad, start - pad + 1).upper()
                ...
                while True:
                    ...
                    if i >= chrom_lengths[chrom] - 3:
                        break
                if (
                    ref_fasta.fetch(chrom, new_end + pad, new_end + pad + 1).upper()
                    == ref_fasta.fetch(chrom, new_end - 1 + pad, new_end + pad).upper()
                ):
                    new_end += 1
                if (
                    ref_fasta.fetch(chrom, new_start - pad, new_start - pad + 1).upper()
                    == ref_fasta.fetch(
                        chrom, new_start - pad + 1, new_start - pad + 2
                    ).upper()
                ):
                    new_start -= 1
                seq, new_seq = (
                    ref_fasta.fetch(chrom, start - pad, end + pad + 1).upper(),
        ...
        bedtools_sort(tmp_, output_fn=extended_region_bed_file, run_logger=logger)
...
def long_read_indelrealign(
    work,
    input_bam,
    output_bam,
    output_vcf,
    output_not_realigned_bed,
    region_bed_file,
    ref_fasta_file,
    num_threads,
    pad,
    ...
    if not output_bam and not output_vcf:
        logger.error("At least one of --output_bam or --output_vcf should be provided.")
        raise Exception
    ...
    while True:
        ...
        len_merged = len_tmp
    shutil.copyfile(region_bed_merged, os.path.join(work, "regions_merged.bed"))

    target_regions = read_tsv_file(region_bed_merged, fields=range(3))
    target_regions = list(map(lambda x: [x[0], int(x[1]), int(x[2])], target_regions))
    ...
    if get_var:
        with open(output_vcf, "w") as o_f:
            ...
            realign_variants = sorted(
                realign_variants, key=lambda x: [chroms_order[x[0]], x[1]]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_vcf",
        type=str,
        help="output_vcf (needed for variant prediction)",
        default=None,
    )
    parser.add_argument(
        "--output_not_realigned_bed",
        type=str,
        help="output_not_realigned_bed",
        required=True,
    )
    parser.add_argument(
        "--output_bam",
        type=str,
        help="output_bam (needed for getting the realigned bam)",
        default=None,
    )
    parser.add_argument("--region_bed", type=str, help="region_bed", required=True)
    parser.add_argument(
    ...
    parser.add_argument(
        "--pad", type=int, help="#base padding to the regions", default=1
    )
    parser.add_argument(
        "--chunk_size", type=int, help="chuck split size for high depth", default=600
    )
    parser.add_argument(
        "--chunk_scale", type=float, help="chuck scale size for high depth", default=1.5
    )
    parser.add_argument(
        "--snp_min_af", type=float, help="SNP min allele freq", default=0.05
    )
    parser.add_argument(
        "--ins_min_af", type=float, help="INS min allele freq", default=0.05
    )
    parser.add_argument(
        "--del_min_af", type=float, help="DEL min allele freq", default=0.05
    )
    parser.add_argument("--match_score", type=int, help="match score", default=10)
    parser.add_argument(
        "--mismatch_penalty", type=int, help="penalty for having a mismatch", default=8
    )
    parser.add_argument(
        "--gap_open_penalty", type=int, help="penalty for opening a gap", default=8
    )
    parser.add_argument(
        "--gap_ext_penalty", type=int, help="penalty for extending a gap", default=6
    )
    parser.add_argument(
        "--max_realign_dp",
        type=int,
        help="max coverage for realign region",
        default=1000,
    )
    parser.add_argument(
        "--do_split",
    ...
    parser.add_argument(
        "--msa_binary", type=str, help="MSA binary", default="../bin/msa"
    )
    args = parser.parse_args()
    logger.info(args)

    try:
        processor = long_read_indelrealign(
            args.work,
            args.input_bam,
            args.output_bam,
            args.output_vcf,
            args.output_not_realigned_bed,
            args.region_bed,
            args.reference,
            args.num_threads,
            args.pad,
        ...
        logger.error("long_read_indelrealign.py failure on arguments: {}".format(args))
        raise e
