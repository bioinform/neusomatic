...
def get_info_value(info_field, variable, ith_alt=None):
    ...
    key_item = re.search(r"\b{}=([^;\s]+)([;\W]|$)".format(variable), info_field)
...
def get_mutect2_info(filters, info, ith_alt):
    ...
    mutect_classification = (
        1 if (get_info_value(info, "SOMATIC") or "PASS" in filters) else 0
    )
...
def get_varscan2_info(info):
    varscan_classification = 1 if get_info_value(info, "SOMATIC") else 0
    return varscan_classification


def get_somaticsniper_info(fields, samples, idxT):
    somaticsniper_classification = (
        1 if get_sample_value(fields, samples, "SS", idxT) == "2" else 0
    )
    if somaticsniper_classification == 1:
        score_somaticsniper = get_sample_value(fields, samples, "SSC", idxT)
    ...
    if (filters == "PASS") and ("Somatic" in info):
        ...
        disqualifying_filters = (
            ...
            or (
                ...
                and float(get_info_value(info, "SSF")) >= 0.15
            )
            or (
...
def read_callers_vcf(
                    ...
                    if ref != alt:
                        if len(ref) == 1 or len(alt) == 1:
                            (
                                ...
                                tandem,
                                ecnt,
                            ) = get_mutect2_info(filters, info, ith_alt)
        ...
        for vardict_vcf in vardict_vcfs:
            ...
            for line in skip_empty(i_f):
                ...
                (
                    ...
                    shift3,
                    score_vardict,
                ) = get_vardict_info(filters, info, fields, samples)
                for alt in alts.split(","):
                    if ref != alt:
                        if (
                            "TYPE=SNV" in info
                            or "TYPE=Deletion" in info
                            or "TYPE=Insertion" in info
                        ):
                            var_id = "-".join([chrom, pos, ref, alt])
                            vardict_info[var_id] = [
                                vardict_classification,
                            ...
                            for i, (ref_i, alt_i) in enumerate(zip(ref, alt)):
                                if ref_i != alt_i:
                                    var_id = "-".join(
                                        [chrom, str(int(pos) + i), ref_i, alt_i]
                                    )
                                    vardict_info[var_id] = [
                                        vardict_classification,
                                        msi,
        ...
        for somaticsniper_vcf in somaticsniper_vcfs:
            ...
            for line in skip_empty(i_f):
                ...
                (
                    somaticsniper_classification,
                    score_somaticsniper,
                ) = get_somaticsniper_info(fields, samples, idxT)
                for alt in alts.split(","):
                    if ref != alt:
                        var_id = "-".join([chrom, pos, ref, alt])
    ...
    features = {}
    for var_id in (
        set(mutect2_info.keys())
        | set(strelka2_info.keys())
        | set(vardict_info.keys())
        | set(varscan2_info.keys())
        | set(somaticsniper_info.keys())
        | set(muse_info.keys())
    ):
        num_callers = 0
        if var_id in mutect2_info:
            mutect_classification, nlod, tlod, tandem, ecnt = mutect2_info[var_id]
    ...
    ordered_vars = sorted(
        features.keys(),
        key=lambda x: [chrom_order["-".join(x.split("-")[:-3])], int(x.split("-")[1])],
    )
    ...
    with open(output_tsv, "w") as o_f:
        ...
        for var_id in ordered_vars:
            ...
            o_f.write(
                "\t".join(
                    [chrom, pos, ".", ref, alt]
                    + list(map(lambda x: str(x).replace("nan", "0"), f))
                )
                + "\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mutect2_vcfs", type=str, nargs="*", help="MuTect2 VCFs", default=None
    )
    parser.add_argument(
        "--strelka2_vcfs", type=str, nargs="*", help="Strelka2 VCFs", default=None
    )
    parser.add_argument(
        "--varscan2_vcfs", type=str, nargs="*", help="VarScan2 VCFs", default=None
    )
    parser.add_argument(
        "--muse_vcfs", type=str, nargs="*", help="MuSE VCFs", default=None
    )
    parser.add_argument(
        "--vardict_vcfs", type=str, nargs="*", help="VarDict VCFs", default=None
    )
    parser.add_argument(
        "--somaticsniper_vcfs",
        type=str,
    ...
    try:
        ...
        logger.error("read_callers_vcf.py failure on arguments: {}".format(args))
        raise e
