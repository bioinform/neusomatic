...
def get_read_pos_for_ref_pos(read, ref_pos_s):
    ...
    cigar_e = (
        (len(cigartuples) - 1)
        if cigartuples[-1][0] == cigar_soft_clip
        else len(cigartuples)
    )
    ...
    for op, length in cigartuples[cigar_s:cigar_e]:
        ...
        while current_i < len(ref_pos_s):
            ...
            if diff < delta_r:
                output[ref_pos_s[current_i]] = [
                    count + diff,
                    (pos_q + diff) if delta_q else None,
                    read.seq[(pos_q + diff)] if delta_q else None,
                    read.query_qualities[(pos_q + diff)] if delta_q else None,
                ]
                current_i += 1
            else:
                break
...
class AugmentedAlignedRead:
    def __init__(self, read, vars_pos):
        ...
        self.is_soft_clipped = (
            read.cigar[0][0] == cigar_soft_clip or read.cigar[-1][0] == cigar_soft_clip
        )
...
class ClusterReads:
    ...
    def get_alignment_features(
        self, var_index, ref_base, first_alt, min_mq=1, min_bq=10
    ):
        """
...
class AlignmentFeatures:
    def __init__(self, reads, my_coordinate, ref_base, first_alt, min_mq=1, min_bq=10):
        ...
        for read_i in reads:
            dp += 1
            read_pos_for_ref_pos = read_i.read_pos_for_ref_pos[my_coordinate[1] - 1]
            (
                code_i,
                ith_base,
                base_call_i,
                indel_length_i,
                flanking_indel_i,
            ) = read_i.pos_of_aligned_read[my_coordinate[1] - 1]
        ...
        for one_count in map(
            lambda x: x.count(1),
            filter(lambda y: len(y) == 2, qname_collector.values()),
        ):
