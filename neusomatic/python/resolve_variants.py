...
def extract_del(record):
    ...
    first_sc = 1 if cigartuples[0][0] in [CIGAR_SOFTCLIP, CIGAR_HARDCLIP] else 0
    last_sc = 1 if cigartuples[-1][0] in [CIGAR_SOFTCLIP, CIGAR_HARDCLIP] else 0
...
class Variant:
    ...
    def var_str(self):
        return "-".join(
            map(str, [self.chrom, self.pos, self.ref, self.alt, self.vtype])
        )
    ...
    def var_gt_str(self):
        return "-".join(
            map(str, [self.chrom, self.pos, self.ref, self.alt, self.gt, self.vtype])
        )
...
def resolve_group(ref_fasta, variants, vars_count):
    ...
    for var_str in vars_count:
        ...
        pos = int(pos)
        v = Variant(chrom, pos, ref, alt, "0/0", 0, vars_count[var_str], vtype)
    ...
    for pos in group_vars:
        ...
        group_vars[pos] = []
        for var_id in var_:
            group_vars[pos].append(
                sorted(var_[var_id], key=lambda x: x.score, reverse=True)[0]
            )
    ...
    for pos in group_vars.keys():
        if max(map(lambda x: x.cnt, group_vars[pos])) < 0.2 * max_count:
            continue
        mx = max(map(lambda x: x.cnt, group_vars[pos]))
        gts = [x.gt for x in group_vars[pos]]
        gts = set([x.gt for x in group_vars[pos] if x.gt != "0/0" or x.len >= 3])
        if len(gts) == 0:
            continue
        if len(gts) > 1:
            ...
            sorted_gts = sorted(
                gts_count.keys(),
                key=lambda x: [gts_count[x], gts_score[x], priority[x]],
                reverse=True,
            )
            gt = sorted_gts[0]
        else:
        ...
        all_vars = sorted(
            group_vars[pos], key=lambda x: [x.cnt, x.score, x.gt != "0/0"], reverse=True
        )
        vtypes = set(
            [
                x.vtype
        ...
        if not vtypes:
            vtypes = set(
                [x.vtype for x in group_vars[pos] if (x.gt != "0/0" or x.len >= 3)]
            )
        all_vars = list(filter(lambda x: x.vtype in vtypes, all_vars))
        if not all_vars:
            logger.info("No vars: {}".format(list(str(x) for x in group_vars[pos])))
            logger.info(
                "No vars: {}".format(
                    [[list(str(x) for x in group_vars[pos_])] for pos_ in group_vars]
                )
            )
            raise Exception
        ...
        if gt == "0/0":
            nz_vars = [
                x for x in all_vars if x.gt != "0/0" and x.vtype == all_vars[0].vtype
            ]
            if nz_vars:
                nz_vars = sorted(nz_vars, key=lambda x: [x.score], reverse=True)[0]
                gt = nz_vars.gt
        v = all_vars[0]
        out_variants_.append([v.chrom, v.pos, v.ref, v.alt, gt, score, v.cnt])

    if (
        len(out_variants_) == 1
        and out_variants_[0][4] == "0/0"
        and abs(len(out_variants_[0][2]) - len(out_variants_[0][3])) >= 3
    ):
        ...
        if resolve_candids:
            resolve_candids = sorted(
                resolve_candids, key=lambda x: [x.score], reverse=True
            )[0]
            out_variants_ = [
                [
                    chrom_,
    ...
    for chrom_, pos_, ref_, alt_, gt_, score_, cnt_ in out_variants_:
        if gt_ not in vars_gt:
            vars_gt[gt_] = []
        vars_gt[gt_].append(Variant(chrom_, pos_, ref_, alt_, gt_, score_, cnt_, ""))
    vars_gt = {
        gt_: sorted(vars_gt[gt_], key=lambda x: [x.cnt, x.score], reverse=True)
        for gt_ in vars_gt
    }
    out_variants_ = []
    for gt_ in vars_gt:
        ...
        for v in vars_gt[gt_][1:]:
            keep = True
            for g_v in good_vs:
                if min(v.pos + len(v.ref), g_v.pos + len(g_v.ref)) > max(
                    v.pos, g_v.pos
                ):
                    keep = False
                    break
            if keep:
                good_vs.append(v)
        for v in good_vs:
            out_variants_.append([v.chrom, v.pos, v.ref, v.alt, v.gt, v.score])
...
def find_resolved_variants(input_record):
    ...
    thread_logger = logging.getLogger(
        "{} ({})".format(
            find_resolved_variants.__name__, multiprocessing.current_process().name
        )
    )
    try:
            ...
            for record in samfile.fetch(chrom, start, end):
                ...
                if not record.is_duplicate or not filter_duplicate:
                    ...
                    if len(aligned_pairs) == 0:
                        continue
                    near_pos = np.where(
                        (start <= aligned_pairs[:, 1]) & (aligned_pairs[:, 1] <= end)
                    )[0]
                    if len(near_pos) != 0:
        ...
        dels = list(
            filter(lambda x: (start <= x[1] <= end) or start <= x[2] <= end, dels)
        )
        if dels:
            del_strs = []
            cnt_ = {}
                    ...
                    list(
                        map(
                            ...
                            [
                                chrom,
                                int(st),
                                ref_fasta.fetch(chrom, st - 1, en).upper(),
                                ref_fasta.fetch(chrom, st - 1, st).upper(),
                                "DEL",
                            ],
                        )
                    )
        ...
        inss = list(
            filter(lambda x: (start <= x[1] <= end) or start <= x[2] <= end, inss)
        )
        if inss:
            cnt_ = {}
            ins_strs = []
                    ...
                    list(
                        map(
                            ...
                            [
                                ...
                                int(st),
                                ref_fasta.fetch(chrom, st - 1, st).upper(),
                                ref_fasta.fetch(chrom, st - 1, st).upper() + bases,
                                "INS",
                            ],
                        )
                    )
        ...
        if snps:
            ...
            for snp_ in uniq_snps:
                ...
                snp_str = "-".join(list(map(str, [chrom, st, ref_, alt_, "SNP"])))
...
def resolve_variants(
    ...
    if num_threads > 1:
        try:
            ...
            while i < len(map_args):
                ...
                out_variants_list.extend(
                    pool.map_async(
                        find_resolved_variants, map_args[batch_i_s:batch_i_e]
                    ).get()
                )
                i = batch_i_e
                pool.close()
    ...
    out_variants = sorted(out_variants, key=lambda x: [chroms_order[x[0]], int(x[1])])
                ...
                o_f.write(
                    "\t".join(
                        [
                            ...
                            ref,
                            alt,
                            "{:.4f}".format(np.round(phred_score, 4)),
                            ".",
                            "SCORE={:.4f}".format(prob),
                            "GT",
                            gt,
                        ]
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_bam", type=str, help="input bam", required=True)
    parser.add_argument("--resolved_vcf", type=str, help="resolved_vcf", required=True)
    parser.add_argument(
        "--target_vcf", type=str, help="resolve target vcf", required=True
    )
    parser.add_argument(
        "--target_bed", type=str, help="resolve target bed", required=True
    )
    parser.add_argument(
    ...
    try:
        ...
        logger.error("resolve_variants.py failure on arguments: {}".format(args))
        raise e
