...
def add_vcf_info(
            ...
            for line in e_f:
                if "POS" in line:
                    ...
                    if "T_DP" in line:
                        ...
                        dp_id, ref_fw_id, ref_rv_id, alt_fw_id, alt_rv_id = (
                            None,
                            None,
                            None,
                            None,
    ...
    with open(output_vcf, "w") as o_f:
        ...
        o_f.write('##FORMAT=<ID=GT,Number=1,Type=String,Description="Genotype">\n')
        o_f.write(
        ...
        for tag in tags:
            ...
            info_field = "{};DP={};RO={};AO={};AF={}".format(score, dp, ro, ao, af)
...
def postprocess(
    ...
    if not long_read:
        ...
        bedtools_intersect(
            ...
            not_resolved_bed,
            args=" -u ",
            output_fn=not_resolved_vcf,
            run_logger=logger,
        )

        all_no_resolve = concatenate_files(
            [no_resolve, ensembled_preds, not_resolved_vcf],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Preprocess predictions for call mode")
    parser.add_argument(
    ...
    parser.add_argument("--pred_vcf", type=str, help="predicted vcf", required=True)
    parser.add_argument(
        "--output_vcf", type=str, help="output final vcf", required=True
    )
    parser.add_argument(
        "--candidates_vcf", type=str, help="filtered candidate vcf", required=True
    )
    parser.add_argument(
        "--ensemble_tsv",
        type=str,
    ...
    parser.add_argument(
        "--postprocess_max_dist",
        type=int,
        help="max distance to neighboring variant",
        default=5,
    )
    parser.add_argument(
        "--long_read",
    ...
    parser.add_argument(
        "--lr_pad",
        type=int,
        help="long_read indel realign: #base padding to the regions",
        default=1,
    )
    parser.add_argument(
        "--lr_chunk_size",
        type=int,
        help="long_read indel realign: chuck split size for high depth",
        default=600,
    )
    parser.add_argument(
        "--lr_chunk_scale",
        type=float,
        help="long_read indel realign: chuck scale size for high depth",
        default=1.5,
    )
    parser.add_argument(
        "--lr_snp_min_af",
        type=float,
        help="long_read indel realign: SNP min allele freq",
        default=0.05,
    )
    parser.add_argument(
        "--lr_ins_min_af",
        type=float,
        help="long_read indel realign: INS min allele freq",
        default=0.05,
    )
    parser.add_argument(
        "--lr_del_min_af",
        type=float,
        help="long_read indel realign: DEL min allele freq",
        default=0.05,
    )
    parser.add_argument(
        "--lr_match_score",
        type=int,
        help="long_read indel realign: match score",
        default=10,
    )
    parser.add_argument(
        "--lr_mismatch_penalty",
        type=int,
        help="long_read indel realign: penalty for having a mismatch",
        default=8,
    )
    parser.add_argument(
        "--lr_gap_open_penalty",
        type=int,
        help="long_read indel realign: penalty for opening a gap",
        default=8,
    )
    parser.add_argument(
        "--lr_gap_ext_penalty",
        type=int,
        help="long_read indel realign: penalty for extending a gap",
        default=6,
    )
    parser.add_argument(
        "--lr_max_realign_dp",
        type=int,
        help="long read max coverage for realign region",
        default=1000,
    )
    parser.add_argument(
        "--lr_do_split",
    ...
    try:
        output_vcf = postprocess(
            ...
            args.lr_del_min_af,
            args.lr_match_score,
            args.lr_mismatch_penalty,
            args.lr_gap_open_penalty,
            args.lr_gap_ext_penalty,
            args.lr_max_realign_dp,
            args.lr_do_split,
            args.keep_duplicate,
            args.pass_threshold,
        ...
        logger.error("postprocess.py failure on arguments: {}".format(args))
        raise e
