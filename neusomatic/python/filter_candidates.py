...
def filter_candidates(candidate_record):
    ...
    thread_logger = logging.getLogger(
        "{} ({})".format(
            filter_candidates.__name__, multiprocessing.current_process().name
        )
    )
    try:
        thread_logger.info(
        ...
        with open(candidates_vcf) as v_f:
            for line in skip_empty(v_f):
                ...
                if loc not in records:
                    records[loc] = []
                if ref == "N" or "\t".join(line.split()[0:5]) not in map(
                    lambda x: "\t".join(x[-1].split()[0:5]), records[loc]
                ):
                    records[loc].append(
                        [
                            chrom,
                ...
                elif "\t".join(line.split()[0:5]) in map(
                    lambda x: "\t".join(x[-1].split()[0:5]), records[loc]
                ):
                    for i, x in enumerate(records[loc]):
                        if "\t".join(line.split()[0:5]) == "\t".join(
                            x[-1].split()[0:5]
        ...
        for loc, rs in sorted(records.items(), key=lambda x: x[1][0:2]) + [
            ...
            if dels and (
                ins or len(list(filter(lambda x: x[3] == "N" and x[2] != "N", rs))) == 0
            ):
                # emit del
                if len(dels) == 1:
                    ro = dels[0][5]
                    ...
                    dels = list(
                        filter(lambda x: x[6] / float(x[5] + x[6]) >= merge_r_thr, dels)
                    )
            ...
            for record in rs:
                ...
                if record[2] != "N" and record[3] != "N" and record[2] != record[3]:
                    bq = record[8]
                    if (
                        ao / float(ro + ao) >= (snp_min_af) or ao >= snp_min_ao
                    ) and bq >= snp_min_bq:
                        # emit SNP
                    ...
                    if ao / float(ro + ao) >= (del_merge_min_af) or ao >= good_ao:
                        ...
                        if dels and pos - dels[-1][1] != 1:
                            ...
                            if len(dels) == 1:
                                ...
                                afs = list(
                                    map(lambda x: x[6] / float(x[5] + x[6]), dels)
                                )
                                max_af = max(afs)
                                merge_r_thr = merge_r * max_af
                                dels = list(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--candidates_vcf", type=str, help="raw candidates vcf", required=True
    )
    parser.add_argument(
        "--filtered_candidates_vcf",
        type=str,
        help="filtered candidates vcf",
        required=True,
    )
    parser.add_argument(
    ...
    try:
        output = filter_candidates(
            (
                args.candidates_vcf,
                args.filtered_candidates_vcf,
                args.reference,
                args.min_dp,
                args.max_dp,
                args.good_ao,
        ...
        logger.error("filter_candidates.py failure on arguments: {}".format(args))
        raise e
