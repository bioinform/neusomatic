...
def get_variant_matrix_tabix(
    ref_seq, count_info, record, matrix_base_pad, chrom_lengths
):
...
def align_tumor_normal_matrices(
    ...
    tumor_col_pos_map[max(tumor_col_pos_map.keys()) + 1] = tumor_matrix_.shape[1]
    normal_col_pos_map[max(normal_col_pos_map.keys()) + 1] = normal_matrix_.shape[1]
    ...
    pN = list(map(lambda x: normal_col_pos_map[x], sorted(normal_col_pos_map.keys())))

    if pT[0] != pN[0]:
        logger.error("record: {}".format(record))
        logger.error("pT, pN: {}, {}".format(pT, pN))
        raise (RuntimeError("pT[0] != pN[0]"))
    ...
    if current_col_T != current_col_N:
        logger.error("record: {}".format(record))
        raise (RuntimeError("current_col_T != current_col_N"))
    ...
    for iii in range(len(tag_tumor_matrices_)):
        new_tag_tumor_matrices_[iii][0, :] = max(tag_tumor_matrices_[iii][0, :])
    ...
    for iii in range(len(tag_normal_matrices_)):
        new_tag_normal_matrices_[iii][0, :] = max(tag_normal_matrices_[iii][0, :])
    ...
    new_tumor_col_pos_map = {k: map_T[v] for k, v in tumor_col_pos_map.items()}
    new_normal_col_pos_map = {k: map_N[v] for k, v in normal_col_pos_map.items()}

    if sum(new_normal_ref_array - new_tumor_ref_array) != 0:
        logger.error("record: {}".format(record))
        logger.error(
            "new_normal_ref_array, new_tumor_ref_array: {}, {}".format(
    ...
    return [
        new_tumor_matrix_,
        new_bq_tumor_matrix_,
        new_mq_tumor_matrix_,
        new_st_tumor_matrix_,
        new_lsc_tumor_matrix_,
        new_rsc_tumor_matrix_,
        new_tag_tumor_matrices_,
        new_normal_matrix_,
        new_bq_normal_matrix_,
        new_mq_normal_matrix_,
        new_st_normal_matrix_,
        new_lsc_normal_matrix_,
        new_rsc_normal_matrix_,
        new_tag_normal_matrices_,
        new_tumor_ref_array,
...
def prepare_info_matrices_tabix(
    ref_seq,
    tumor_count_info,
    normal_count_info,
    record,
    ...
    bq_normal_matrix_[0, np.where(normal_matrix_.sum(0) == 0)[0]] = np.max(
        bq_normal_matrix_
    )
    mq_tumor_matrix_[0, np.where(tumor_matrix_.sum(0) == 0)[0]] = np.max(
        mq_tumor_matrix_
    )
    mq_normal_matrix_[0, np.where(normal_matrix_.sum(0) == 0)[0]] = np.max(
        mq_normal_matrix_
    )
    st_tumor_matrix_[0, np.where(tumor_matrix_.sum(0) == 0)[0]] = np.max(
        st_tumor_matrix_
    )
    st_normal_matrix_[0, np.where(normal_matrix_.sum(0) == 0)[0]] = np.max(
        st_normal_matrix_
    )
    lsc_tumor_matrix_[0, np.where(tumor_matrix_.sum(0) == 0)[0]] = np.max(
        lsc_tumor_matrix_
    )
    lsc_normal_matrix_[0, np.where(normal_matrix_.sum(0) == 0)[0]] = np.max(
        lsc_normal_matrix_
    )
    rsc_tumor_matrix_[0, np.where(tumor_matrix_.sum(0) == 0)[0]] = np.max(
        rsc_tumor_matrix_
    )
    rsc_normal_matrix_[0, np.where(normal_matrix_.sum(0) == 0)[0]] = np.max(
        rsc_normal_matrix_
    )
    for iii in range(len(tag_tumor_matrices_)):
        tag_tumor_matrices_[iii][0, np.where(tumor_matrix_.sum(0) == 0)[0]] = np.max(
            tag_tumor_matrices_[iii]
        )
    for iii in range(len(tag_normal_matrices_)):
        tag_normal_matrices_[iii][0, np.where(normal_matrix_.sum(0) == 0)[0]] = np.max(
    ...
    normal_matrix_[0, np.where(normal_matrix_.sum(0) == 0)[0]] = max(
        np.sum(normal_matrix_, 0)
    )
    ...
    if n_row < min_cov:
        logger.warning("Skip {} for low cov {}<{}".format(record, int(n_row), min_cov))
        return None
    cols_not_to_del = []
    largest_block = []
    if len(alt) > len(ref):
        ...
        if z_ref_array.shape[0] > 0:
            ...
            cols_not_to_del = np.where(
                np.logical_and(count_column >= max_count * 0.7, (ref_array == 0))
            )[0]
            ...
            blocks = list(
                map(
                    lambda x: [a[x[0]], a[x[1]]],
                    zip(np.where(b == 1)[0], np.where(b == -1)[0]),
                )
            )
            if blocks:
                largest_block = sorted(blocks, key=lambda x: x[1] - x[0] + 1)[-1]
                if (
                    np.max(count_column[range(largest_block[0], largest_block[1] + 1)])
                    > max_count * 0.05
                ):
                    if (largest_block[1] - largest_block[0] + 1) > 2:
                        cols_not_to_del = sorted(
                            list(
                                set(cols_not_to_del)
    ...
    cols_to_del = sorted(
        list(
            set(
                np.where(
                    np.logical_and(
                        count_column <= (min_ev_frac_per_col * n_row), (ref_array == 0)
                    )
                )[0]
            )
            - set(cols_not_to_del)
    ...
    if n_col - len(cols_to_del) > tw:
        mn = min(
            count_column[np.argsort(count_column)][n_col - tw - 1],
            max(n_row // 5, min_ev_frac_per_col * n_row),
        )
        new_cols_to_del = (
            set(np.where(np.logical_and(count_column <= mn, (ref_array == 0)))[0])
            - set(cols_to_del)
            - set(cols_not_to_del)
        )
        if (
            n_col - (len(cols_to_del) + len(new_cols_to_del)) < tw
            and len(new_cols_to_del) > 3
            and len(alt) > len(ref)
        ):
            new_cols_to_del = list(map(lambda x: [count_column[x], x], new_cols_to_del))
    ...
    if n_col - len(cols_to_del) > tw:
        for i in range(n_col // 10):
            if i not in cols_to_del and sum(rcenter) > -3:
                ref_b = ref_array[i]
                if tumor_matrix_[ref_b, i] > 0.8 * n_row:
            ...
            if ii not in cols_to_del and sum(rcenter) < 3:
                ref_b = ref_array[ii]
                if tumor_matrix_[ref_b, ii] > 0.8 * n_row:
        ...
        if block_len > 2:
                ...
                list(
                    ...
                    | set(
                        range(
                            largest_block[0],
                            largest_block[0]
                            + min(n_col - len(cols_to_del) - tw, block_len - 3),
                        )
                    )
                )
            )
            ...
            cols_to_del = sorted(
                list(
                    set(cols_to_del)
                    | (
                        set(np.where(count_column <= (min_ev_frac_per_col * n_row))[0])
                        & set(range(largest_block[0], largest_block[1] + 1))
                    )
                )
            )
    ...
    tag_tumor_matrices = []
    for iii in range(len(tag_tumor_matrices_)):
        tag_tumor_matrices.append(np.delete(tag_tumor_matrices_[iii], cols_to_del, 1))
    ...
    tag_normal_matrices = []
    for iii in range(len(tag_normal_matrices_)):
        tag_normal_matrices.append(np.delete(tag_normal_matrices_[iii], cols_to_del, 1))
    ...
    if matrix_width >= ncols:
        col_pos_map = {
            i: v + (matrix_width - ncols) // 2 for i, v in col_pos_map.items()
        }
        tumor_count_matrix = np.zeros((5, matrix_width))
        tumor_count_matrix[
            :, (matrix_width - ncols) // 2 : (matrix_width - ncols) // 2 + ncols
        ] = tumor_matrix
        ...
        col_pos_map = {
            i: int(round(v / float(ncols) * matrix_width))
            for i, v in col_pos_map.items()
        }
        tumor_count_matrix = np.array(
            Image.fromarray(tumor_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        bq_tumor_count_matrix = np.array(
            Image.fromarray(bq_tumor_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        mq_tumor_count_matrix = np.array(
            Image.fromarray(mq_tumor_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        st_tumor_count_matrix = np.array(
            Image.fromarray(st_tumor_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        lsc_tumor_count_matrix = np.array(
            Image.fromarray(lsc_tumor_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        rsc_tumor_count_matrix = np.array(
            Image.fromarray(rsc_tumor_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        ...
        for iii in range(len(tag_tumor_matrices)):
            tag_tumor_count_matrices.append(
                np.array(
                    Image.fromarray(tag_tumor_matrices[iii]).resize(
                        (matrix_width, 5), 2
                    )
                ).astype(int)
            )

        normal_count_matrix = np.array(
            Image.fromarray(normal_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        bq_normal_count_matrix = np.array(
            Image.fromarray(bq_normal_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        mq_normal_count_matrix = np.array(
            Image.fromarray(mq_normal_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        st_normal_count_matrix = np.array(
            Image.fromarray(st_normal_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        lsc_normal_count_matrix = np.array(
            Image.fromarray(lsc_normal_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        rsc_normal_count_matrix = np.array(
            Image.fromarray(rsc_normal_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
        ...
        for iii in range(len(tag_normal_matrices)):
            tag_normal_count_matrices.append(
                np.array(
                    Image.fromarray(tag_normal_matrices[iii]).resize(
                        (matrix_width, 5), 2
                    )
                ).astype(int)
            )
        ref_count_matrix = np.array(
            Image.fromarray(ref_matrix).resize((matrix_width, 5), 2)
        ).astype(int)
    ...
    if center > ref_count_matrix.shape[1] - 1:
        center = min(
            max(0, min(col_pos_map.values()) + rcenter[0] - 1 + rcenter[1]),
...
def prep_data_single_tabix(input_record):
    ...
    thread_logger = logging.getLogger(
        "{} ({})".format(
            prep_data_single_tabix.__name__, multiprocessing.current_process().name
        )
    )
    try:
        ...
        matrices_info = prepare_info_matrices_tabix(
            ref_seq=ref_seq,
            tumor_count_info=tumor_count_info,
            normal_count_info=normal_count_info,
            record=record,
            rlen=rlen,
            rcenter=rcenter,
        ...
        candidate_mat = np.zeros(
            (
                ...
                tumor_count_matrix.shape[1],
                13 + (len(tag_tumor_count_matrices) * 2),
            )
        )
        ...
        for iii in range(len(tag_tumor_count_matrices)):
            ...
            candidate_mat[:, :, 13 + (iii * 2) + 1] = tag_normal_count_matrices[iii]
        ...
        candidate_mat[:, :, 0] = (
            candidate_mat[:, :, 0]
            / (max(np.max(ref_count_matrix), np.max(tumor_count_matrix)) + 0.00001)
            * max_norm
        )
        candidate_mat[:, :, 1] = (
            candidate_mat[:, :, 1] / (np.max(tumor_count_matrix) + 0.00001) * max_norm
        )
        candidate_mat[:, :, 2] = (
            candidate_mat[:, :, 2] / (np.max(normal_count_matrix) + 0.00001) * max_norm
        )
        candidate_mat[:, :, 3] = (
            candidate_mat[:, :, 3]
            / (max(np.max(bq_tumor_count_matrix), 41.0))
            * max_norm
        )
        candidate_mat[:, :, 4] = (
            candidate_mat[:, :, 4]
            / (max(np.max(bq_normal_count_matrix), 41.0))
            * max_norm
        )
        candidate_mat[:, :, 5] = (
            candidate_mat[:, :, 5]
            / (max(np.max(mq_tumor_count_matrix), 70.0))
            * max_norm
        )
        candidate_mat[:, :, 6] = (
            candidate_mat[:, :, 6]
            / (max(np.max(mq_normal_count_matrix), 70.0))
            * max_norm
        )
        candidate_mat[:, :, 7] = (
            candidate_mat[:, :, 7] / (np.max(tumor_count_matrix) + 0.00001) * max_norm
        )
        candidate_mat[:, :, 8] = (
            candidate_mat[:, :, 8] / (np.max(normal_count_matrix) + 0.00001) * max_norm
        )
        candidate_mat[:, :, 9] = (
            candidate_mat[:, :, 9] / (np.max(tumor_count_matrix) + 0.00001) * max_norm
        )
        candidate_mat[:, :, 10] = (
            candidate_mat[:, :, 10] / (np.max(normal_count_matrix) + 0.00001) * max_norm
        )
        candidate_mat[:, :, 11] = (
            candidate_mat[:, :, 11] / (np.max(tumor_count_matrix) + 0.00001) * max_norm
        )
        candidate_mat[:, :, 12] = (
            candidate_mat[:, :, 12] / (np.max(normal_count_matrix) + 0.00001) * max_norm
        )
        for iii in range(len(tag_tumor_count_matrices)):
            candidate_mat[:, :, 13 + (iii * 2)] = (
                candidate_mat[:, :, 13 + (iii * 2)]
                / (max(np.max(tag_tumor_count_matrices[iii]), 100.0))
                * max_norm
            )
            candidate_mat[:, :, 13 + (iii * 2) + 1] = (
                candidate_mat[:, :, 13 + (iii * 2) + 1]
                / (max(np.max(tag_normal_count_matrices[iii]), 100.0))
                * max_norm
            )
...
def push_lr(fasta_file, record, left_right_both):
    ...
    if "," not in record[3]:
        if record[2] != record[3] and left_right_both in [0, 2]:
            ...
            new_alt = alt
            while new_pos > 1:
                l_base = fasta_file.fetch((chrom), new_pos - 2, new_pos - 1).upper()
        ...
        if record[2] != record[3] and left_right_both:
            ...
            while new_pos < max_pos:
                r_base = fasta_file.fetch((chrom), new_pos - 1, new_pos).upper()
                ...
                if len(new_alt) > len(alt):
                    ...
                    eqs.append(
                        [chrom, new_pos - len(new_ref), new_ref, new_alt] + record[4:]
                    )
            record = [chrom, new_pos - len(new_ref), new_ref, new_alt] + record[4:]
        eqs = list(
            map(
                lambda x: eqs[x],
                dict(
                    map(lambda x: ["_".join(map(str, x[1])), x[0]], enumerate(eqs))
                ).values(),
            )
        )
...
def is_part_of(record1, record2):
    ...
    if vartype1 == "SNP" and vartype2 == "DEL":
        ...
        if pos1 == pos2:
            return True
        elif vartype1 == "DEL" and set(range(pos1 + 1, pos1 + len(ref1))) & set(
            range(pos2 + 1, pos2 + len(ref2))
        ):
            return True
    return False
...
def find_len(ref, alt):
    ...
    i_ = 0
    while (
        min(len(alt_), len(ref_)) > i_
        and alt_[len(alt_) - i_ - 1] == ref_[len(ref_) - i_ - 1]
    ):
        i_ += 1
    if i_ > 0:
        ref_ = ref_[:-i_]
...
def keep_in_region(input_file, region_bed, output_fn):
    ...
    good_i = set([])
    tmp_ = bedtools_window(tmp_, region_bed, args=" -w 1", run_logger=logger)
...
def find_records(input_record):
    ...
    try:
        ...
        split_truth_vcf_file = os.path.join(work, "truth_{}.vcf".format(work_index))
        ...
        bedtools_intersect(
            ...
            split_bed,
            args=" -u",
            output_fn=split_truth_vcf_file,
        ...
        keep_in_region(
            input_file=tmp_, region_bed=split_region_file, output_fn=split_pred_vcf_file
        )
        if ensemble_bed:
            tmp_ = get_tmp_file()
            bedtools_intersect(
            ...
            concatenate_vcfs(
                [split_pred_vcf_file, split_missed_ensemble_bed_file],
            ...
            bedtools_sort(
                tmp_, output_fn=split_pred_with_missed_file, run_logger=thread_logger
            )
            ...
            in_ensemble_bed = bedtools_window(
                split_pred_with_missed_file,
                split_ensemble_bed_file,
                output_fn=split_in_ensemble_bed,
                args=" -w 1",
                run_logger=thread_logger,
            )
            ...
            with open(in_ensemble_bed) as ni_f:
                for line in skip_empty(ni_f):
                    ...
                    if curren_pos_records:
                        if (
                            record[0] == curren_pos_records[0][0]
                            and record[1] == curren_pos_records[0][1]
                            and record[3] == curren_pos_records[0][3]
                            and record[4] == curren_pos_records[0][4]
                        ):
                    ...
                    if emit_flag:
                        if curren_pos_records:
                            ...
                            for record_ in curren_pos_records:
                                ...
                                ens_chrom, ens_pos, ens_ref, ens_alt = [
                                    str(record_[10]),
                                    int(record_[11]),
                                    record_[13],
                                    record_[14],
                                ...
                                if pos == ens_pos:
                                    if ref == ens_ref and alt == ens_alt:
                                        ...
                                        len(ref) > len(alt)
                                        and len(ens_ref) > len(ens_alt)
                                        and (alt) == (ens_alt)
                                    ):
                                        if (
                                            len(ref) > len(ens_ref)
                                            and ref[0 : len(ens_ref)] == ens_ref
                                        ) or (
                                            len(ens_ref) > len(ref)
                                            and ens_ref[0 : len(ref)] == ref
                                        ):
                                            ann = record_[15:]
                                    elif (
                                        len(ref) < len(alt)
                                        ...
                                        if (
                                            len(alt) > len(ens_alt)
                                            and alt[0 : len(ens_alt)] == ens_alt
                                        ) or (
                                            len(ens_alt) > len(alt)
                                            and ens_alt[0 : len(alt)] == alt
                                        ):
                                            ann = record_[15:]
                                if ann:
                            ...
                            if has_var_match:
                                rrs = list(filter(lambda x: x[2], rrs))[0:1]
        ...
        with open(records_bed, "w") as r_b:
            for x in records:
                r_b.write(
                    "\t".join(
                        map(str, [x[0], x[1], x[1] + len(x[2]), x[2], x[3], x[4]])
                    )
                    + "\n"
                )
        ...
        with open(split_truth_vcf_file, "r") as vcf_reader:
            for line in skip_empty(vcf_reader):
                ...
                pos = int(record[1])
                if (
                    len(record[3]) != len(record[4])
                    and min(len(record[3]), len(record[4])) > 0
                    and record[3][0] != record[4][0]
                ):
                    if pos > 1:
        ...
        for i, js in map_truth_2_pred.items():
            ...
            for j in js:
                record = records[j]
                if (
                    record[0] == truth_record[0]
                    and record[1] == truth_record[1]
                    and record[2] == truth_record[2]
                    and record[3] == truth_record[3]
                ):
        ...
        remained_idx = sorted(
            set(range(len(records))) - (set(good_records_idx) | set(none_records_ids))
        )
        ...
        for i, js in map_truth_2_pred.items():
            ...
            if len(js) == 2 and vartype_t == "SNP":
                ...
                if (vartype0 == "DEL" and vartype1 == "INS") or (
                    vartype1 == "DEL" and vartype0 == "INS"
                ):
                    for j in js:
                        record = records[j]
                        pos, ref, alt = record[1:4]
        ...
        for i, js in map_truth_2_pred.items():
            ...
            for j in js:
                ...
                if (
                    vartype_t == vartype
                    and pos_t == pos
                    and ((not strict_labeling) or vartype_t != "SNP")
                ):
                    good_records[vartype_t].append(j)
                    vtype[j] = vartype_t
                    record_len[j] = find_len(ref_t, alt_t)
            ...
            for i, js in map_truth_2_pred.items():
                ...
                for j in js:
                    ...
                    if pos_t + rct[0] + rct[1] == pos + rc[0] + rc[1]:
                        if (vartype_t == "INS" and vartype == "SNP") or (
                            vartype == "INS" and vartype_t == "SNP"
                        ):
                            good_records[vartype_t].append(j)
                            vtype[j] = vartype_t
                            record_len[j] = find_len(ref_t, alt_t)
...
def extract_ensemble(
    ...
    logger = logging.getLogger(
        "{} ({})".format(
            extract_ensemble.__name__, multiprocessing.current_process().name
        )
    )
    ...
    for ensemble_tsv in ensemble_tsvs:
        with open(ensemble_tsv) as s_f:
            for line in skip_empty(s_f):
                if line.startswith("CHROM"):
                    ...
                    if custom_header and not is_extend:
                        ...
                        if set(expected_features) - set(header):
                            logger.error(
                                "The following features are missing from ensemble file {}: {}".format(
                                    ensemble_tsv,
                ...
                features = list(
                    map(
                        lambda x: float(x.replace("False", "0").replace("True", "1")),
                        features,
                    )
                )
                if custom_header and not is_extend:
    ...
    if not custom_header or is_extend:
        ...
        mq_features = list(
            map(
                ...
                filter(
                    lambda x: x[1]
                    in ["nBAM_REF_MQ", "nBAM_ALT_MQ", "tBAM_REF_MQ", "tBAM_ALT_MQ"],
                    enumerate(header),
                ),
            )
        )
        bq_features = list(
            map(
                ...
                filter(
                    lambda x: x[1]
                    in ["nBAM_REF_BQ", "nBAM_ALT_BQ", "tBAM_REF_BQ", "tBAM_ALT_BQ"],
                    enumerate(header),
                ),
            )
        )
        nm_diff_features = list(
            map(
                lambda x: x[0],
                filter(
                    lambda x: x[1] in ["nBAM_NM_Diff", "tBAM_NM_Diff"],
                    enumerate(header),
                ),
            )
        )
        ranksum_features = list(
            map(
                lambda x: x[0],
                filter(
                    lambda x: x[1]
        ...
        stralka_scor = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["Strelka_Score"], enumerate(header)),
            )
        )
        stralka_qss = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["Strelka_QSS"], enumerate(header)),
            )
        )
        stralka_tqss = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["Strelka_TQSS"], enumerate(header)),
            )
        )
        varscan2_score = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["VarScan2_Score"], enumerate(header)),
            )
        )
        vardict_score = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["VarDict_Score"], enumerate(header)),
            )
        )
        m2_lod = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["M2_NLOD", "M2_TLOD"], enumerate(header)),
            )
        )
        sniper_score = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["Sniper_Score"], enumerate(header)),
            )
        )
        m2_ecent = list(
            map(
                lambda x: x[0], filter(lambda x: x[1] in ["M2_ECNT"], enumerate(header))
            )
        )
        sor = list(
            map(lambda x: x[0], filter(lambda x: x[1] in ["SOR"], enumerate(header)))
        )
        msi = list(
            map(lambda x: x[0], filter(lambda x: x[1] in ["MSI"], enumerate(header)))
        )
        msilen = list(
            map(lambda x: x[0], filter(lambda x: x[1] in ["MSILEN"], enumerate(header)))
        )
        shift3 = list(
            map(lambda x: x[0], filter(lambda x: x[1] in ["SHIFT3"], enumerate(header)))
        )
        MaxHomopolymer_Length = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["MaxHomopolymer_Length"], enumerate(header)),
            )
        )
        SiteHomopolymer_Length = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["SiteHomopolymer_Length"], enumerate(header)),
            )
        )
        InDel_Length = list(
            map(
                lambda x: x[0],
                filter(lambda x: x[1] in ["InDel_Length"], enumerate(header)),
            )
        )
        Seq_Complexity_ = list(
            map(
                lambda x: x[0],
                filter(
                    lambda x: x[1] in ["Seq_Complexity_Span", "Seq_Complexity_Adj"],
                    enumerate(header),
                ),
            )
        )
    ...
    with open(ensemble_bed, "w") as f_:
        f_.write("#" + "\t".join(map(str, header_pos + selected_features_tags)) + "\n")
...
def parallel_generation(inputs):
    ...
    thread_logger = logging.getLogger(
        "{} ({})".format(
            parallel_generation.__name__, multiprocessing.current_process().name
        )
    )
    ...
    try:
        ...
        for w in map_args:
            ...
            if chrom not in chrom_pos:
                ...
                chrom_pos[chrom] = [
                    min(s_pos, chrom_pos[chrom][0]),
                    max(e_pos, chrom_pos[chrom][1]),
                ]
        ...
        for chrom in chrom_pos:
            ...
            try:
                tumor_tabix_records = list(
                    tb_tumor.fetch(chrom, chrom_pos[chrom][0] - 1, chrom_pos[chrom][1])
                )
            except:
                thread_logger.warning(
                    "No count information at {} for {}:{}-{}".format(
                        tumor_count_bed,
                        chrom,
                        chrom_pos[chrom][0] - 1,
                        chrom_pos[chrom][1],
                ...
                tumor_tabix_records = []
            try:
                normal_tabix_records = list(
                    tb_normal.fetch(chrom, chrom_pos[chrom][0] - 1, chrom_pos[chrom][1])
                )
            except:
                thread_logger.warning(
                    "No count information at {} for {}:{}-{}".format(
        ...
        for w in map_args:
            ...
            e_pos = min(pos + matrix_base_pad, chrom_lengths[chrom] - 2)
            tumor_counts = {
                x_pos: tumor_tabix_records_dict[chrom][x_pos]
                for x_pos in range(s_pos, e_pos + 1)
                if x_pos in tumor_tabix_records_dict[chrom]
            }
            normal_counts = {
                x_pos: normal_tabix_records_dict[chrom][x_pos]
                for x_pos in range(s_pos, e_pos + 1)
                if x_pos in normal_tabix_records_dict[chrom]
            }
            w[1] = tumor_counts
            w[2] = normal_counts
...
def generate_dataset(
    ...
    logger = logging.getLogger(
        "{} ({})".format(
            generate_dataset.__name__, multiprocessing.current_process().name
        )
    )
    ...
    if ensemble_tsv and not ensemble_bed:
        ...
        extract_ensemble(
            ensemble_tsvs=[ensemble_tsv],
            ensemble_bed=ensemble_bed,
            no_seq_complexity=no_seq_complexity,
            enforce_header=enforce_header,
    ...
    split_region_files = split_region(
        work, region_bed_file, num_splits, shuffle_intervals=False
    )
    ...
    with open(var_vcf, "w") as vv, open(none_vcf, "w") as nv:
        ...
        while is_ < candidates_split:
            ...
            logger.info(
                "Write {}/{} split to {} for cnts ({}..{})/{}".format(
                    is_ + 1,
                    candidates_split,
                    candidates_tsv_file,
                    is_current,
                    ...
                    for is_none, records in [
                        ...
                        for record in records:
                            ...
                            if is_current <= cnt < is_end:
                                ...
                                s_pos = max(1, pos - matrix_base_pad)
                                e_pos = min(
                                    pos + matrix_base_pad, chrom_lengths[chrom] - 2
                                )
                                ref_seq = (
                                    fasta_file.fetch(chrom, s_pos - 1, e_pos)
                                    .upper()
                                    .replace("N", "-")
                                )

                                map_args_records.append(
                                    [
                                        ref_seq,
                                        None,
                                        None,
                        ...
                        with multiprocessing.Pool(num_threads) as pool:
                            records_done_ = pool.map_async(
                                ...
                                [
                                    ...
                                    for i_split in range(0, len_records, split_len)
                                ],
                            ).get()
                    except Exception as inst:
                        logger.error(inst)
            ...
            with open(candidates_tsv_file, "w") as b_o:
                for x in records_done:
                    if x:
                        ...
                        b_o.write(
                            "\t".join(
                                [str(cnt_ims), "1", tag, compressed_candidate_mat]
                                + list(map(lambda x: str(np.round(x, 4)), ann))
                            )
                            + "\n"
                        )
                        cnt_ims += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--truth_vcf",
        type=str,
        help="truth vcf (required for train mode)",
        default=None,
    )
    parser.add_argument(
        "--tumor_pred_vcf",
        type=str,
        help="tumor candidate variants vcf file",
        required=True,
    )
    ...
    parser.add_argument(
        "--tumor_count_bed",
        type=str,
        help="tumor count bed.gz tabix file",
        required=True,
    )
    parser.add_argument(
        "--normal_count_bed",
        type=str,
        help="normal count bed.gz tabix file",
        required=True,
    )
    parser.add_argument(
        "--reference", type=str, help="reference fasta filename", required=True
    )
    parser.add_argument("--work", type=str, help="work directory", required=True)
    parser.add_argument(
        "--tsv_batch_size", type=int, help="output files batch size", default=50000
    )
    parser.add_argument(
        "--matrix_window_size", type=int, help="target window width", default=32
    )
    parser.add_argument(
        "--matrix_base_pad",
        type=int,
        help="number of bases to pad around the candidate variant",
        default=7,
    )
    parser.add_argument(
        "--min_ev_frac_per_col",
        type=float,
        help="minimum frac cov per column to keep columm",
        default=0.06,
    )
    ...
    parser.add_argument(
        "--ensemble_tsv",
        type=str,
        help="Ensemble annotation tsv file (only for short read)",
        default=None,
    )
    parser.add_argument(
        "--ensemble_bed",
        type=str,
        help="Ensemble annotation bed file (only for short read)",
        default=None,
    )
    parser.add_argument(
        "--ensemble_custom_header",
    ...
    parser.add_argument(
        "--zero_vscore", help="set VarScan2_Score to zero", action="store_true"
    )
    parser.add_argument(
        "--matrix_dtype",
        type=str,
    ...
    parser.add_argument(
        "--strict_labeling", help="strict labeling in train mode", action="store_true"
    )
    ...
    try:
        ...
        logger.error("generate_dataset.py failure on arguments: {}".format(args))
        raise e
