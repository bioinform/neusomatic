...
def run_scan_alignments(record):
    thread_logger = logging.getLogger(
        "{} ({})".format(
            run_scan_alignments.__name__, multiprocessing.current_process().name
        )
    )
    ...
    try:
        ...
        if merge_d_for_scan is not None:
            ...
            bedtools_merge(
                tmp_,
                output_fn=split_region_file_,
                args=" -d {}".format(merge_d_for_scan),
        ...
        if os.path.getsize(split_region_file_) > 0:
            ...
            run_shell_command(
                cmd,
                stdout=os.path.join(work, "scan.out"),
        ...
        return (
            os.path.join(work, "candidates.vcf"),
            os.path.join(work, "count.bed.gz"),
        ...
        if os.path.exists(stderr_file) and os.path.getsize(stderr_file):
            thread_logger.error("Please check error log at {}".format(stderr_file))
        return None

        outputs = scan_alignments(
            args.work,
            args.merge_d_for_scan,
            args.scan_alignments_binary,
            args.input_bam,
            args.regions_bed_file,
            args.reference,
            args.num_splits,
            args.num_threads,
            args.window_size,
            ...
            args.report_count_for_all_positions,
...
def scan_alignments(
    ...
    logger = logging.getLogger(
        "{} ({})".format(
            scan_alignments.__name__, multiprocessing.current_process().name
        )
    )
    ...
    if not split_region_files:
        ...
        if not split_region_files:
            ...
            if num_splits is not None:
                ...
                num_split = max(
                    int(np.ceil((total_len // 10000000) // num_threads) * num_threads),
                    num_threads,
                )
    ...
    for i, split_region_file in enumerate(split_region_files):
        if (
            restart
            or not os.path.exists(os.path.join(work, "work.{}".format(i), "region.bed"))
            ...
            map_args.append(
                (
                    os.path.join(work, "work.{}".format(i)),
                    reference,
                    merge_d_for_scan,
                    scan_alignments_binary,
                    split_region_file,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--regions_bed_file", type=str, help="regions bed file", default=""
    )
    parser.add_argument(
        "--scan_alignments_binary",
        type=str,
    ...
    parser.add_argument(
        "--window_size", type=int, help="window size to scan the variants", default=2000
    )
    parser.add_argument(
        "--snp_min_ao",
        type=float,
    ...
    parser.add_argument(
        "--ins_min_af", type=float, help="INS min allele freq", default=0.01
    )
    parser.add_argument(
        "--del_min_af", type=float, help="DEL min allele freq", default=0.01
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--report_count_for_all_positions",
        help="report_count_for_all_positions",
        action="store_true",
    )
    parser.add_argument(
    ...
    try:
        ...
        logger.error("scan_alignments.py failure on arguments: {}".format(args))
        raise e
